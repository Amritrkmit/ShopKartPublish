...
import { ArrowLeft, Printer, RefreshCw, FileDown, Wand2, Image as ImageIcon, XCircle } from "lucide-react";
...
const OrderDetailsAdmin = () => {
    const { id } = useParams();
    // Decrypt the ID from the URL (if valid base64 encrypted)
    // If not encrypted (legacy), decryptId returns null or bad data, so we can try fallback or use as-is if decrypt fails but that logic is inside decryptId which returns null on error. 
    // Since we forced encryption, we expect encrypted. But for backward compat, if decrypt fails, use raw?
    // User wants standardization. Let's assume always encrypted now.
    // Actually our decryptId returns null on fail. We should handle raw ID just in case during transition or dev.
    ...
    const fetchSingleOrder = useCallback(async () => {
        try {
            const res = await axios.get(`${API_BASE_URL}/admin/orders/${decryptedId}`, {
    ...
    const handleUpdateStatus = async () => {
        try {
            await axios.put(
                `${API_BASE_URL}/admin/orders/${decryptedId}`,
                {
        ...
        if (rawItems) {
            ...
            while (typeof rawItems === 'string' && attempts < 5) {
                try {
                    ...
                    try {
                        const sanitized = rawItems.trim().replace(/\\"/g, '"');
                        if (sanitized.startsWith('"') && sanitized.endsWith('"')) {
                            rawItems = JSON.parse(sanitized.slice(1, -1));
                        } else {
                            rawItems = JSON.parse(sanitized);
                        }
                    } catch (e2) {
                        console.error("Failed to parse items even with sanitization", e2);
                        break;
                    }
                }
                attempts++;
            }
            items = Array.isArray(rawItems) ? rawItems : (typeof rawItems === 'object' && rawItems !== null ? Object.values(rawItems) : []);
        }
    } catch (e) {
        console.error("Critical error parsing items", e);
    }

    // Calculate totals from order data
    const itemsSubtotal = items.reduce((sum, item) => sum + (parseFloat(item.sale_price) * (item.quantity || item.qty || 1)), 0);
    ...
    return (
        <div className="p-4 max-w-7xl mx-auto print:p-0 font-sans">
            {/* Header */}
            <div className="flex items-center justify-between mb-4 print:hidden">
                <Link to="/admin/orders/" className="flex items-center text-gray-500 hover:text-gray-700 transition-colors text-xs font-bold uppercase tracking-wider">
                ...
                <div className="flex gap-2">
                    ...
                    <button className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-all font-bold text-[10px] uppercase tracking-widest shadow-sm">
                ...
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <div className="text-[11px] text-gray-500 flex items-center gap-1.5 font-medium uppercase tracking-tight">
                            Customer:
                            <Link to={`/admin/users?search=${order.user_email || order.user_id}`} className="text-blue-600 hover:underline font-black">
                                {order.user_name || "Guest"} ({order.user_email || order.user_id || "N/A"})
                            </Link>
                        </div>
                    </div>
                    <div className="text-right">
                        <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-1">Payment Method</p>
                        <p className="text-[11px] font-black text-gray-700 bg-gray-50 px-3 py-1 rounded-lg border border-gray-100 uppercase tracking-wider">
                            {getPaymentMode(order.payment_id)}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <div className="px-5 py-2 border-b border-gray-100 bg-gray-50/30">
                            <h2 className="text-[11px] font-black text-gray-900 uppercase tracking-widest">Ordered Products</h2>
                        </div>
