...
import FeatureManager from "../../admin/components/FeatureManager";
...
export default function SellerProductUpload() {
        ...
        try {
            ...
            if (product) {
                setFormData({
                    ...
                    available_sizes: product.available_sizes ? JSON.parse(product.available_sizes) : [],
                ...
                if (product.image) setPreview(`${API_BASE_URL.replace("/api", "")}${product.image.replace(/^\/?assets/, "/assets")}`);
    ...
    const fetchAttributes = useCallback(async (catId, subCatId) => {
        if (!catId) return;
        try {
            let url = `${API_BASE_URL}/attributes?category_id=${catId}`;
    ...
    const fetchBundles = useCallback(async () => {
        if (!decryptedId) return;
        try {
            const res = await axios.get(`${API_BASE_URL}/bundles/${decryptedId}`);
    ...
    const addBundle = async (p) => {
        ...
        try {
            const token = localStorage.getItem("sellerToken") || localStorage.getItem("userToken");
            await axios.post(`${API_BASE_URL}/bundles/${decryptedId}`, { bundle_product_id: p.id }, {
    ...
    const removeBundle = async (bundleId) => {
        ...
        try {
            const token = localStorage.getItem("sellerToken") || localStorage.getItem("userToken");
            await axios.delete(`${API_BASE_URL}/bundles/${decryptedId}/${bundleId}`, {
                headers: token ? { Authorization: `Bearer ${token}` } : {},
                withCredentials: true
            });
            setBundleProducts(prev => prev.filter(p => p.id !== bundleId));
    ...
    }, [decryptedId, fetchVariants, confirm]);
    ...
    useEffect(() => {
        ...
        if (selectedSub) setSubcategorySearch(selectedSub.name);
        else setSubcategorySearch("");
    }, [formData.subcategory_id, subcategories]);

    useEffect(() => {
        if (formData.brand) setBrandSearch(formData.brand);
    }, [formData.brand]);
    ...
    const addField = (type) => {
        const fields = [...getCustomizationFields()];
        const labels = { text: "Message / Name", textarea: "Instruction Box", date: "Event Date", image: "Reference Image" };
        const newField = { name: `${type}_${Date.now()}`, type: type, label: labels[type] || "New Field", required: false, placeholder: `Enter ${labels[type] || "details"}...` };
    ...
    const handleAttributeCheckbox = (attrName, optionValue, isChecked) => {
        setFormData(prev => {
            ...
            const newArray = isChecked ? [...currentValues, optionValue] : currentValues.filter(v => v !== optionValue);
            return { ...prev, attributes: { ...prev.attributes, [attrName]: newArray } };
        });
    };

    const filteredCategories = categories.filter(cat => cat.name.toLowerCase().includes(categorySearch.toLowerCase()));
    const filteredSubcategories = subcategories.filter(sub => sub.name.toLowerCase().includes(subcategorySearch.toLowerCase()));
    ...
    const handleSaveDraft = async () => await handleSubmit(null, "draft");
    const handlePublish = async () => await handleSubmit(null, "published");

    const handleSubmit = async (e, statusOverride) => {
        if (e) e.preventDefault();

        // Ensure Seller is present
        if (!seller || !seller.shop_id) {
            toastError("Session invalid. Please refresh.");
            return;
        }
        ...
        // Inject Seller Shop ID
        data.append("shop_id", seller.shop_id);
        ...
        try {
            ...
            if (isEditMode) {
                await axios.put(`${API_BASE_URL}/products/${decryptedId}`, data, {
            ...
            if (!isEditMode && productId) {
                ...
                for (const bundle of bundleProducts) {
                    try {
                        await axios.post(`${API_BASE_URL}/bundles/${productId}`, { bundle_product_id: bundle.id }, { headers, withCredentials: true });
                ...
                <div className="max-w-7xl mx-auto px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                            <span className="hover:text-blue-600 cursor-pointer" onClick={() => navigate("/seller/products")}>Products</span>
                            <ChevronRight size={16} />
                            <span className="text-gray-900 font-semibold">{isEditMode ? "Edit product" : "Add a product"}</span>
                        </div>
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="px-5 py-2 text-sm font-medium text-white bg-[#dc3545] rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50"
                            >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            ...
                            <input
                                ...
                                className={`w-full px-4 py-3 text-sm border rounded-lg focus:outline-none focus:ring-2 transition-colors ${errors.name ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-blue-500'}`}
                            />
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            ...
                            <div className="flex items-center gap-1 mb-3 pb-3 border-b border-gray-200">
                                <button type="button" className="p-2 hover:bg-gray-100 rounded"><Bold size={16} className="text-gray-600" /></button>
                                <button type="button" className="p-2 hover:bg-gray-100 rounded"><Italic size={16} className="text-gray-600" /></button>
                                <button type="button" className="p-2 hover:bg-gray-100 rounded"><Underline size={16} className="text-gray-600" /></button>
                                <div className="w-px h-6 bg-gray-300 mx-1"></div>
                                <button type="button" className="p-2 hover:bg-gray-100 rounded"><List size={16} className="text-gray-600" /></button>
                                <button type="button" className="p-2 hover:bg-gray-100 rounded"><ListOrdered size={16} className="text-gray-600" /></button>
                                <div className="w-px h-6 bg-gray-300 mx-1"></div>
                                <button type="button" className="p-2 hover:bg-gray-100 rounded"><LinkIcon size={16} className="text-gray-600" /></button>
                            </div>
                            <textarea
                                name="description"
                                ...
                                <label htmlFor="image-upload" className="cursor-pointer">
                                    {preview ? (
                                        <div className="relative inline-block">
                                            ...
                                            <button type="button" onClick={(e) => { e.preventDefault(); setImage(null); setPreview(null); setDeleteMainImage(true); }} className="absolute -top-2 -right-2 p-1 bg-red-500 text-white rounded-full hover:bg-red-600"><X size={16} /></button>
                                        </div>
                                    ) : (
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors cursor-pointer mb-4">
                                <input type="file" accept="image/*" multiple onChange={handleGalleryChange} className="hidden" id="gallery-upload" />
                                <label htmlFor="gallery-upload" className="cursor-pointer w-full h-full block">
                                    <div className="flex flex-col items-center gap-2">
                                        <div className="bg-blue-50 p-3 rounded-full"><Layers size={24} className="text-blue-600" /></div>
                                        <p className="text-sm text-gray-600"><span className="text-blue-600 font-medium">Add images</span> to gallery</p>
                                    </div>
                                </label>
                            </div>
                                    ...
                                    <div className="grid grid-cols-4 gap-4">
                                        {existingGalleryImages.map((img) => (
                                            <div key={img.id} className="relative group border rounded-lg overflow-hidden h-24">
                                                <img src={`${API_BASE_URL}${img.image_url.replace(/^\/?assets/, "/assets")}`} alt="" className="w-full h-full object-cover" />
                                                <button type="button" onClick={() => handleDeleteGalleryImage(img.id)} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full hover:bg-red-600"><X size={16} /></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-4 gap-4">
                                        {galleryPreviews.map((src, idx) => (
                                            <div key={idx} className="relative group border rounded-lg overflow-hidden h-24">
                                                ...
                                                <button type="button" onClick={() => removeGalleryImage(idx)} className="absolute -top-2 -right-2 p-1 bg-red-500 text-white rounded-full hover:bg-red-600"><X size={16} /></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        {/* Feature Manager */}
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            <FeatureManager features={formData.product_features} onChange={(newFeatures) => setFormData(prev => ({ ...prev, product_features: newFeatures }))} />
                        </div>

                        {/* Inventory Section (Standard/Admin Component) */}
                        <InventorySection
                            formData={formData}
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="meta_title" value={formData.meta_title} onChange={handleChange} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="SEO Title" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Meta Description</label>
                                    <textarea name="meta_description" value={formData.meta_description} onChange={handleChange} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" rows={3} placeholder="SEO Description..." />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Meta Keywords</label>
                                    <input type="text" name="meta_keywords" value={formData.meta_keywords} onChange={handleChange} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Comma separated keywords" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            ...
                            <select name="status" value={formData.status} onChange={handleChange} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            type="text" placeholder="Search category..." value={categorySearch}
                                            onChange={(e) => { setCategorySearch(e.target.value); setShowCategoryDropdown(true); }}
                                            onFocus={() => setShowCategoryDropdown(true)}
                                            data-error={!!errors.category_id}
                                            className={`w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 ${errors.category_id ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-blue-500'}`}
                                        />
                                            ...
                                            <>
                                                ...
                                                <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                    {filteredCategories.length > 0 ? filteredCategories.map(cat => (
                                                        <div key={cat.id} onClick={() => handleCategorySelect(cat.id, cat.name)} className={`px-3 py-2 cursor-pointer hover:bg-blue-50 text-sm ${formData.category_id === cat.id ? 'bg-blue-100' : ''}`}>{cat.name}</div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            type="text" placeholder={!formData.category_id ? "Select category first" : "Search subcategory..."}
                                            ...
                                            <>
                                                ...
                                                <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                    {filteredSubcategories.map(sub => (
                                                        <div key={sub.id} onClick={() => handleSubcategorySelect(sub.id, sub.name)} className={`px-3 py-2 cursor-pointer hover:bg-blue-50 text-sm ${formData.subcategory_id === sub.id ? 'bg-blue-100' : ''}`}>{sub.name}</div>
                                                    ))}
                                                </div>
                                            </>
                                        )}
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3 bg-gray-50 p-4 rounded-lg border border-gray-200">
                                        <div className="relative">
                                            <input type="text" placeholder="Search products..." value={variantSearch} onChange={(e) => { setVariantSearch(e.target.value); searchProducts(e.target.value); }} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                            {searchResults.length > 0 && (
                                                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                    {searchResults.map(p => (
                                                        <div key={p.id} onClick={() => handleAddVariant(p.id)} className="flex items-center gap-3 p-3 hover:bg-gray-50 cursor-pointer border-b last:border-b-0">
                                                            <img src={`${API_BASE_URL}${p.image}`} className="w-8 h-8 object-cover rounded" alt="" />
                                        ...
                                        {variants.length > 0 && <div className="space-y-2">{variants.map(v => (
                                            <div key={v.id} className="flex items-center gap-2 p-2 bg-white border rounded">
                                                <img src={`${API_BASE_URL}${v.image}`} className="w-8 h-8 rounded" alt="" />
                                                <span className="text-xs flex-1 line-clamp-1">{v.name}</span>
                                                <button onClick={() => handleRemoveVariant(v.id)} className="text-red-500"><Trash2 size={14} /></button>
                                            </div>
                                        ))}</div>}
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input type="text" placeholder="Search brand..." value={brandSearch || formData.brand} onChange={(e) => { setBrandSearch(e.target.value); setFormData(p => ({ ...p, brand: e.target.value })); setShowBrandDropdown(true); }} onFocus={() => setShowBrandDropdown(true)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                        {showBrandDropdown && (
                                            <>
                                                ...
                                                <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                    {brands.filter(b => b.name.toLowerCase().includes((brandSearch || "").toLowerCase())).map(b => (
                                                        <div key={b.id} onClick={() => handleBrandSelect(b.name)} className="px-3 py-2 hover:bg-blue-50 cursor-pointer text-sm">{b.name}</div>
                                                    ))}
                                                </div>
                                            </>
                                        )}
                                        ...
                                        {[
                                            ...
                                            <label key={segment.id} className="flex items-center gap-2 cursor-pointer">
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        const current = formData.tags ? formData.tags.split(',').filter(Boolean) : [];
                                                        const newTags = e.target.checked ? [...current, segment.id] : current.filter(t => t !== segment.id);
                        ...
                        {/* Bundles */}
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            <div className="flex items-center gap-2 mb-4"><Layers size={18} /><h3 className="text-sm font-semibold">Frequently Bought Together</h3></div>
                            <div className="relative mb-4">
                                <input type="text" placeholder="Search products..." value={bundleSearch} onChange={(e) => handleBundleSearch(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                {bundleSearchResults.length > 0 && (
                                    <div className="absolute z-20 w-full mt-1 bg-white border rounded-lg shadow-xl max-h-60 overflow-y-auto">
                                        {bundleSearchResults.map(p => (
                                            <div key={p.id} onClick={() => addBundle(p)} className="px-4 py-2 hover:bg-gray-50 cursor-pointer flex justify-between items-center text-sm">
                                                <span>{p.name}</span><span className="text-blue-600 font-bold text-xs">ADD</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                {bundleProducts.map(p => (
                                    <div key={p.id} className="flex justify-between items-center p-2 bg-gray-50 rounded border">
                                        <div className="flex items-center gap-2">
                                            <img src={`${API_BASE_URL}${p.image}`} className="w-8 h-8 rounded" alt="" />
                                            <span className="text-xs line-clamp-1">{p.name}</span>
                                        </div>
                                        <button onClick={() => removeBundle(p.id)} className="text-red-500"><X size={14} /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        {/* Similar Products */}
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            <h3 className="text-sm font-semibold mb-4">Manual Similar Products</h3>
                            <div className="relative mb-4">
                                <input type="text" placeholder="Search..." value={similarProductSearch} onChange={(e) => { setSimilarProductSearch(e.target.value); searchSimilarProducts(e.target.value); }} className="w-full px-3 py-2 text-sm border rounded-lg" />
                                {similarSearchResults.length > 0 && (
                                    <div className="absolute z-20 w-full mt-1 bg-white border rounded shadow-xl max-h-60 overflow-y-auto">
                                        {similarSearchResults.map(p => (
                                            <div key={p.id} onClick={() => addSimilarProduct(p)} className="px-4 py-2 hover:bg-gray-50 cursor-pointer flex justify-between text-sm">
                                                <span>{p.name}</span><span className="text-blue-600 text-xs font-bold">ADD</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            <div className="space-y-2">
                                {selectedSimilarProducts.map(p => (
                                    <div key={p.id} className="flex justify-between items-center p-2 bg-gray-50 rounded border">
                                        <span className="text-xs line-clamp-1">{p.name}</span>
                                        <button onClick={() => removeSimilarProduct(p.id)} className="text-red-500"><Trash2 size={14} /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-lg border border-gray-200 p-6">
                            ...
                            <input type="text" name="slug" value={formData.slug} onChange={handleChange} className="w-full px-3 py-2 text-sm border bg-gray-50 rounded-lg" />
                        </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {['S', 'M', 'L', 'XL', 'XXL', 'XXXL'].map(size => (
                                    <button
                                        ...
                                        onClick={() => setFormData(prev => ({ ...prev, available_sizes: prev.available_sizes.includes(size) ? prev.available_sizes.filter(s => s !== size) : [...prev.available_sizes, size] }))}
                                        className={`px-4 py-2 text-sm font-medium rounded-lg border ${formData.available_sizes.includes(size) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300'}`}
                                    >
                                        {size}
                                    </button>
                                ))}
